...
**進捗ステータス（2026-08-19時点）: フェーズ1〜3すべて完了。** 各フェーズの完了条件を満たしており、`numberPlaceService.ts` の内部実装も自作バックトラッキングアルゴリズムへの差し替えが完了している。本ロードマップはフェーズ3までとし、以降の機能追加・変更はGitHub Issueで個別に管理する（本ファイルへの新規フェーズ追記は行わない）。

## フェーズ1: UI構築とライブラリ接続（MVP作成） — ✅ 完了
...
- 外部のパズル解法ライブラリへの依存が `package.json` から削除されている（`src/services/backtracking/solver.ts` と `generator.ts` による自作実装に置き換え済み）

## フェーズ横断の作業指針

- 各タスクは可能な限り「テストを先に書く」小さな単位に分割して進める
- `numberPlaceService.ts` の公開契約を変更する場合は `docs/spec.md` 12章を必ず更新してから着手する
- フェーズ1〜3完了後の機能追加・変更はGitHub Issueで管理する（`CLAUDE.md` の「Issue→PR運用フロー」参照）。本ロードマップへのフェーズ追加は行わない
