...
| 74  | 候補消去型（`kind: 'elimination'`）のヒントの場合は選択マスを変更しない（Issue #26） | `numberPlaceService.findHint` をモックしEliminationHintを返す状況で `requestHint()`          | `hint.status === 'highlight'`、`hint.hint` がモックしたEliminationHintと一致するが、`selected` は変化しない（`null` のまま） |
