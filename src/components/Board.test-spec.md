...
除去系4技法の追加に伴い、`hintPosition?: Position` を `hintCells?: HintCell[]`（各要素が
`{ position, role: 'cause' | 'eliminated' }`）に置き換える。

| #   | ケース                                                                     | 前提・入力                                                                        | 期待される結果                                                              |
| --- | --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| 17  | hintCellsで指定した位置のマスにそれぞれのroleに応じたヒントスタイルが付く | `hintCells: [{position:{row:3,col:5},role:'cause'}, {position:{row:3,col:7},role:'eliminated'}]` | (3,5)が`data-hint="cause"`、(3,7)が`data-hint="eliminated"`。他のマスには付かない |
| 18  | hintCellsが空配列のとき（省略時も同様）はどのマスにもヒントスタイルが付かない | `hintCells: []`（省略時も同様）                                                      | すべてのマスに `data-hint` が付かない                                              |
